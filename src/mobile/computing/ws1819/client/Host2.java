	...
	public static void main(String[] args) throws IOException {
		...
		if(args.length >= 1) {
			for(int i = 0; i < args.length; i++) {
				System.out.println("At Host2 - Server Says:  "+args[i]);
			}
		}
			...
			if (status.equals("j")) {
				...
				try {
					...
					HostRestClient.postHost(Host.createHost2(group));
				...
				group = Integer.parseInt(sc.nextLine());
				try {
					HostRestClient.deleteHost(group , Host.createHost2(group).getId());
